...
public interface UserNodeRepository extends Neo4jRepository<UserNode, Long> {
  ...
  @Query(
    ...
    """)
  List<String> findListsContainingFriend(
  ...
  @Query(
      """
        ...
        RETURN fr
    """)
  List<UserNode> findFriendsByList(
      @Param("userId") Long userId, @Param("listName") String listName);
  ...
  @Query(
      value = "MATCH (u:User {id: $userId})-[:FOLLOWS]->(followed:User) RETURN followed",
      countQuery =
          "MATCH (u:User {id: $userId})-[:FOLLOWS]->(followed:User) RETURN count(followed)")
  Page<UserNode> findFollowingPaginated(@Param("userId") Long userId, Pageable pageable);

  @Query(
      value = "MATCH (follower:User)-[:FOLLOWS]->(u:User {id: $userId}) RETURN follower",
      countQuery =
          "MATCH (follower:User)-[:FOLLOWS]->(u:User {id: $userId}) RETURN count(follower)")
  Page<UserNode> findFollowersPaginated(@Param("userId") Long userId, Pageable pageable);
  ...
  @Query(
      value = "MATCH (u:User {id: $userId})-[:BLOCKS]->(blocked:User) RETURN blocked",
      countQuery = "MATCH (u:User {id: $userId})-[:BLOCKS]->(blocked:User) RETURN count(blocked)")
  Page<UserNode> findBlockedUsersPaginated(@Param("userId") Long userId, Pageable pageable);
