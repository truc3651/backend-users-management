...
public class SecurityConfig {
  ...
  private void configureOpenPaths(HttpSecurity http) throws Exception {
    ...
    http.authorizeHttpRequests(
        auth -> openPaths.forEach(openPath -> permitOpenPath(auth, openPath)));
    http.authorizeHttpRequests(
        auth -> auth.requestMatchers(new AntPathRequestMatcher("**/internal/**")).permitAll());
  }
  ...
  private void permitOpenPath(
    ...
    openPath
        ...
        .forEach(
            method ->
                auth.requestMatchers(new AntPathRequestMatcher(pattern, method.name()))
                    .permitAll());
  }
}
